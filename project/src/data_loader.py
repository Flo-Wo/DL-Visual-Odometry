...
class DatasetFrames(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Load data and get label
        X1 = torch.load(path_tensor_frames + "{:05d}.pt".format(ID))
        X2 = torch.load(path_tensor_frames + "{:05d}.pt".format(ID + 1))
        y = (self.labels[ID] + self.labels[ID + 1]) / 2

        return X1, X2, y


class DatasetOptFloFrames(torch.utils.data.Dataset):
    'Characterizes a dataset for PyTorch'
    ...
    def __getitem__(self, index):
        ...
        # Load data and get label
        F1 = torch.load(path_tensor_frames + "{:05d}.pt".format(ID))
        F2 = torch.load(path_tensor_frames + "{:05d}.pt".format(ID + 1))
        X = torch.load(path_tensor_opt_fl + "{:05d}.pt".format(ID))
        y = (self.labels[ID] + self.labels[ID + 1]) / 2
...
def save_frames_as_tensors(save_path):
    ...
    for i, frame in enumerate(load_single_images(path_raw_video)):
        if i % 100 == 0:
            print(i)
