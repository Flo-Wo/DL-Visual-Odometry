...
We added two generic pooling layers to the network. One after the second convolutional layer and the second one before the fully
connected layers start. We tested maximum and average pooling with the following parameters
\begin{align*}
...
network, we achieved a MSE of under 10 on the test set, which is according to the companies guidelines a pretty good result.
Especially, as we trained the model mostly on highway scenes and tested it only in city driving scenarios.
\begin{table}[!t]
\normalsize
...
\begin{tabular}{lcccc}
\toprule
\multirow{2}{*}{Initial splitting, dropout layer}  & \multicolumn{2}{c}{$\mathrm{ReLU}$} & \multicolumn{2}{c}{$\mathrm{leakyReLU}$} \\
...
\bottomrule
\end{tabular}
\caption{MSE results of the network using pooling and dropout layers, different activation functions and the initial splitting.}
